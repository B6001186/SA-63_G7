...
// The TitlenameQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type TitlenameQueryRuleFunc func(context.Context, *ent.TitlenameQuery) error

// EvalQuery return f(ctx, q).
func (f TitlenameQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.TitlenameQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.TitlenameQuery", q)
}

// The TitlenameMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type TitlenameMutationRuleFunc func(context.Context, *ent.TitlenameMutation) error

// EvalMutation calls f(ctx, m).
func (f TitlenameMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.TitlenameMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.TitlenameMutation", m)
}
