...
// QueryTitlename chains the current query on the titlename edge.
func (eq *EmployeeQuery) QueryTitlename() *TitlenameQuery {
	query := &TitlenameQuery{config: eq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithTitlename tells the query-builder to eager-loads the nodes that are connected to
// the "titlename" edge. The optional arguments used to configure the query builder of the edge.
func (eq *EmployeeQuery) WithTitlename(opts ...func(*TitlenameQuery)) *EmployeeQuery {
	query := &TitlenameQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withTitlename = query
	return eq
}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context) ([]*Employee, error) {
	var (
		...
		loadedTypes = [3]bool{
			...
			eq.withPlace != nil,
			eq.withTitlename != nil,
		}
	)
	if eq.withDepartment != nil || eq.withPlace != nil || eq.withTitlename != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := eq.withTitlename; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Employee)
		for i := range nodes {
			if fk := nodes[i].titlename_employee; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(titlename.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "titlename_employee" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Titlename = n
			}
		}
	}
