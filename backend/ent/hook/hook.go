...
// The TitlenameFunc type is an adapter to allow the use of ordinary
// function as Titlename mutator.
type TitlenameFunc func(context.Context, *ent.TitlenameMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TitlenameFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.TitlenameMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TitlenameMutation", m)
	}
	return f(ctx, mv)
}
