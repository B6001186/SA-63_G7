...
import { EntTitlename } from '../../api/models/EntTitlename';
...
const Employee: FC<{}> = () => {
  ...
  const [titlenames, setTitlenames] = React.useState<EntTitlename[]>([]);
  ...
  const getTitlename = async () => {
    const res = await http.listTitlename({ limit: 5, offset: 0 });
    setTitlenames(res);
  }
